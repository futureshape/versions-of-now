...
#define VIDEO_APLL_CALIBRATION_TIMEOUT_US 10000
...
static void video_apll_enable_with_coefficients(uint32_t sdm0, uint32_t sdm1, uint32_t sdm2, uint32_t o_div)
{
    ...
    for (uint32_t elapsed_us = 0; elapsed_us < VIDEO_APLL_CALIBRATION_TIMEOUT_US; elapsed_us++) {
        if (clk_ll_apll_calibration_is_done()) {
            return;
        }
        esp_rom_delay_us(1);
    }

    ESP_LOGW(TAG, "APLL calibration did not finish after %u us; continuing with configured coefficients",
             VIDEO_APLL_CALIBRATION_TIMEOUT_US);
#else
...
static volatile uint32_t DRAM_ATTR g_visible_frame_counter = 0;
static volatile uint32_t DRAM_ATTR g_frame_counter = 0;
...
void video_wait_frame(void)
{
    ...
    const uint32_t start_frame = g_visible_frame_counter;
    const TickType_t start_ticks = xTaskGetTickCount();
    const TickType_t timeout_ticks = 1000 / portTICK_PERIOD_MS;

    while (g_visible_frame_counter == start_frame && (xTaskGetTickCount() - start_ticks) < timeout_ticks) {
        vTaskDelay(1);
    }
}
